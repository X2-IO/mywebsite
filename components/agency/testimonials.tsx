        ...
        <ul className="mt-14 grid list-none gap-6 p-0 md:grid-cols-3">
          {quotes.map((t, i) => (
            <li key={t.name}>
              <figure
                data-motion-fade
                className="flex h-full flex-col rounded-2xl border border-white/[0.08] bg-zinc-900/40 p-6 backdrop-blur-sm transition-transform duration-500 hover:scale-[1.02] hover:border-white/[0.12]"
                style={{
                  animation:
              ...
              >
                <blockquote className="flex-1 text-sm leading-relaxed text-zinc-300">
                  <span className="sr-only">Asiakaslausunto: </span>
                  &ldquo;{t.quote}&rdquo;
                </blockquote>
                <figcaption className="mt-6 flex items-center gap-3 border-t border-white/[0.06] pt-6">
                  <div
                    className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-zinc-600 to-zinc-800 text-xs font-semibold text-zinc-200"
                    aria-hidden
                  >
                    {t.name
                      .split(" ")
